...
all exist.  That is, we want all triples (x, y, z) such that (x,y), (y,z),
and (x,z) are all in R:

```
{ (x, y, z) : R(x,y) ∧ R(y,z) ∧ R(x,z) }
```

Computing this AND is what database people call a **join**: the three
subexpressions R(x,y), R(y,z), and R(x,z) share variables (x, y, z appear
in multiple places), so evaluating the AND requires matching up tuples on
their shared columns.  The challenge is doing this efficiently.

We'll build up to this in three steps: first a two-relation join (which turns
out to be sparse matrix multiply), then the three-relation triangle query with
different relations, and finally the self-join on a single relation stored in CSR.
...
tuples.  Without values/properties it's a vector of pairs; with values (edge
weights, matrix entries) it's a vector of triples (i, j, v).
...
The trie interface has four operations:
...
and **intersects** their iterators — advancing them in lockstep via `seek` so
that only matching keys survive.  This intersection is the core mechanism:
when iterator A is behind iterator B, seek A forward to B's current value,
and vice versa.
...
The trie-join formalizes what sparse matrix programmers already do.
No intersections are needed here — each level has at most one relation to
iterate, so the join is just nested iteration.

Note something remarkable: **SpMM is a special case of the trie-join** and that realizing it as a trie-join results in a straightforward way to do spMM on a matrix stored in COO format.  The
same framework that will give us worst-case optimal triangle counting also
...
### Approach 2: Worst-case optimal join (trie-join)

A **worst-case optimal join** (WCOJ) processes all relations simultaneously
instead of combining them pairwise.  The idea is mechanical: given the query
R(x,y) ∧ S(y,z) ∧ T(x,z), we generate a single nested loop as follows:
...
3. **At each loop level, intersect the iterators** from all relations that
   mention the current variable.  If a relation mentions a variable from a
   higher level, we've already `open`-ed it at that value, so we're
   iterating its next level.  If a relation first appears at this level,
   we iterate its top level.
...
This achieves the **AGM bound**: for relations of size M each, the triangle
query produces at most O(M^{3/2}) results, and the trie-join runs in
O(M^{3/2}) time — matching the theoretical maximum output size.

| | Pairwise | Trie-join (WCOJ) |
|-|----------|------------------|
| Intermediate | O(M²) in worst case | None — single pass |
| Total cost | O(M²) | O(M^{3/2}) |
| Mechanism | SpMM then filter | Intersect at every level |

The trie-join is **worst-case optimal** (WCOJ): no algorithm can do better
in the worst case, because the output itself can be Θ(M^{3/2}).  This
applies to any relations R, S, T — the self-join R = S = T is a special case.
...
| 2 | intersect(R.open(x), S.level1) | R.open(x)  (subset ∩ universe) |
...
function intersect_neighbors(rowptr, colval, a, a_lo, b, f)
    a_hi = rowptr[a+1] - 1
    b_lo = rowptr[b]
    b_hi = rowptr[b+1] - 1
    ai, bi = a_lo, b_lo
...
function triangle_count(rowptr, colval, N)
    ...
    for x in 1:N                                      # ① R.level1
        for yi in rowptr[x] : rowptr[x+1]-1           # ② R.open(x)
            ...
            #    x-side starts at yi+1 (after y), y-side starts at rowptr[y]
            #    gallop_gt skips entries ≤ y on the y-side
            intersect_neighbors(rowptr, colval,
                x, yi + 1,                            #    neighbors(x) after y
                y,                                    #    neighbors(y)
...
with y > x, intersect the neighbor lists of x and y**.  The
`intersect_neighbors` function is the computational kernel — everything
else is just iteration over edges.
...
WCOJ recipe from Section 3, and generates the nested-loop + intersection
code automatically.
...
| Operation | What it does |
|-----------|-------------|
| `iterate(Val(k))` | Return the next key at trie level k |
| `seek_lub(v, Val(k))` | Jump to first key ≥ v at level k |
| `open(Val(k))` | Save state at level k, descend to level k+1 |
| `close(Val(k))` | Restore state to before the last open at level k |
